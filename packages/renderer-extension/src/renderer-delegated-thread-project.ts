...
/** Apply Desktop's project filters. An ambiguous match must not file the thread under another project. */
export function projectForDelegatedThread(
...
async function loadDesktopOrderBinding(): Promise<DesktopOrderBinding> {
  ...
  return {
    async register(hostId, created) {
      ...
      const projectId = projectForDelegatedThread(
...
/** Add a newly delegated thread to the current project's saved sidebar list. */
export function installRendererDelegatedThreadProject(
  manager: RequestManager,
  load: () => Promise<DesktopOrderBinding> = loadDesktopOrderBinding,
  diagnose: () => void = () =>
    console.warn("codexhost: delegated thread was not added to the current project"),
): (() => void) | null {
