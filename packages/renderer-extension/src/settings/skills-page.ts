...
/** Harness ids are opaque at the contract boundary; only known ones get an icon. */
...
/**
 * A Harness belongs on this page only when it is actually usable here: Codex is
 * always present, and every other Harness has to be connected on this machine.
 * Directory existence is not a substitute — codexhost writes a delegation Skill
 * into `~/.claude/skills` itself, so that directory exists without Claude Code.
 */
  ...
  return Object.freeze({
    ...
    mount(context: RendererSettingsPageMountContext) {
      ...
      const render = (): void => {
        ...
        // Naming the Harnesses that already read the shared directory explains
        // why they have no control, instead of looking like a missing feature.
        ...
        list.replaceChildren();
        for (const skill of matched) list.append(createSkillRow(skill, linkTargets));
