...
import {
  delegationProject,
  findSidebarScope,
  installRendererDelegationOrder,
} from "../src/renderer-delegation-order.js";
...
describe("delegation sidebar order", () => {
  it("uses the most specific native project filter and respects Host and assignment exclusions", () => {
    ...
    expect(delegationProject([parent, child], "local", created)).toBe("parent");
    expect(delegationProject([parent], "remote", created)).toBeNull();
    expect(delegationProject([parent, project("duplicate", "/work")], "local", created)).toBeNull();
    parent.scope.filter.includeThreadIds.push("child");
    expect(delegationProject([parent], "local", { ...created, cwd: "/elsewhere" })).toBe("parent");
  });
  ...
  it("registers only new delegation notifications once, in order, and unsubscribes", async () => {
    ...
    const install = installRendererDelegationOrder(
      {
        getHostId: () => "local",
  ...
  it("does not mutate after disposal or let an unsupported Desktop break later events", async () => {
    ...
    const dispose = installRendererDelegationOrder(
      {
        hostId: "local",
