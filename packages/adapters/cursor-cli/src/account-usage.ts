...
const percent = (value: unknown): number | undefined =>
  typeof value === "number" && Number.isFinite(value) && value >= 0 ? Math.min(100, value) : undefined;
  ...
  const authPath =
    process.platform === "darwin"
      ...
      : process.platform === "win32"
        ...
        : path.join(environment.XDG_CONFIG_HOME ?? path.join(home, ".config"), "cursor", "auth.json");
...
export async function fetchCursorAccount(
  ...
  try {
    ...
    if (environment.CURSOR_API_KEY?.trim() || environment.AGENT_CLI_CREDENTIAL_STORE === "memory")
      return null;
    const token = await (input.readAccessToken ? input.readAccessToken() : accessToken(environment));
