...
// Public Pi provider API preserves native reasoning and service-tier pricing.
...
/** Read the installed Codex model capability cache, never credentials or a guessed GPT list. */
      ...
      value.models
        .filter(
          (m) =>
            ...
            m.service_tiers.some((tier: { id?: string }) => tier?.id === "priority"),
        )
        .map((m) => m.slug),
    );
  } catch {
...
export function piFastValue(ref: HarnessModelRef, supported: ReadonlySet<string>): boolean {
  ...
  return config.values.fast === "true";
}
  ...
  const models = catalog.models.map((model) => {
    ...
    const configurationOptions =
      native.provider === "openai-codex" && supported.has(native.id)
        ...
        : [];
    return { ...model, configurationOptions };
  });
  return {
    ...catalog,
    models,
    configurationOptions:
      models.find((m) => m.ref.id === catalog.defaultModel?.id)?.configurationOptions ?? [],
  };
}
