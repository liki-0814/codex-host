...
设置 → 会话导入可登记 **Claude Code、Pi、Hermes**、**DSH `0.1.2-rc.1` / `0.1.5-rc.1` Session**、**Qoder SDK Session**、**Grok Session** 和 **Cursor CLI ACP Session**。导入只建立 Host Thread 与原生 Session 的映射，不复制 Transcript、不转换 Harness、不发送用户 Turn；打开后仍通过对应 Adapter 的 `open({ kind: "resume" })` 恢复历史并继续会话。
