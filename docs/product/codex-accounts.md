...
在 codexhost 的「设置 → 账号」查看当前 Codex 身份与额度，以及其他 Harness 的 `inspectAccount()` 快照。CodexHost 不管理多个原生 Codex 登录：不提供添加/登录/切换/退出/删除/恢复，也不消耗重置卡。官方 Desktop 登录与退出仍由官方后端处理。设置页不把登录复制到其他 Harness，也不建立 Host 凭据库。

本地 `.codexhost-native-accounts` 文件若仍存在，启动和刷新都不会读取、改写或回收。

## 账号列表

页面上方是「账号 / 剩余或已用 / 管理」三列表格。额度窗口画在中间这一格里，不拆成固定的 5 小时列和 7 天列；表头随「剩余 / 已用」切换。Codex 行在有额度数据时提供逐行刷新，其他 Harness 行保留「原生管理」说明。工具栏另有全局刷新。窄窗口下每个账号独立排列。视觉沿用原设置外壳：无边框搜索。工具栏的「账号」数量包含当前 Codex 账号和实际返回的其他 Harness 账号，不随搜索筛选改变。
...
- 5 小时、7 天和其他原生窗口画在同一额度格内，而不是各占一列表头。缺少的窗口仅显示「—」，不补成已用 0% 或剩余 100%。月额度、模型组及产品专属额度在账号信息下独立具名显示，不冒充全账号总额度，也不合并或丢弃重复报告。
...
统一列表中展示 Grok Build、agy（Antigravity）、Claude Code 当前原生认证可读取的真实额度。其他 Harness 行的最后一列是「原生管理」说明，不提供把登录复制到 Pi 的入口。这不是多账号管理：不提供添加、删除、切换、设为默认或重置卡操作，也不修改 Codex 当前账号。搜索和已用/剩余切换作用于所有行，刷新按钮重新查询两类额度。各 Harness 独立并行查询，任一有效结果返回后立即显示，不等待其他 Harness；全局刷新期间同样逐项恢复。
...
设置页不提供把 Codex 或 Grok 登录复制到 Pi 的入口，账号表也没有「用于 Harness」列。仓库里仍有上游的导入模块，但没有接到 Pi Adapter、Grok Adapter 或设置页。
...
- `packages/renderer-extension/src/settings/accounts-page.ts`：身份与额度；不挂载凭证导入。
...
- `packages/renderer-extension/src/settings/accounts-usage.ts`：同一额度格内的窗口、额外具名额度和重置卡详情。
