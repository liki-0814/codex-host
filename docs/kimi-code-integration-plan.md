...
选择模型、Effort 或权限先更新前端待发送配置；发送时将完整配置应用到原生会话，不因每次选择而重启进程。使用稳定 prompt_id，并在发送响应丢失时查询原生是否已接受，避免重复执行。
...
以上覆盖已列的成功路径，不代表所有故障场景均经过实机测试。跨 Harness 递归委派、多进程同时修改同一原生会话、所有权限组合的每种工具行为尚未完成完整实机组合测试。原生账户当前没有额外额度样本，不推断未知字段。
